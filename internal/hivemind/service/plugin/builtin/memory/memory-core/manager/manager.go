...
import (
	...
	embedding2 "github.com/kiosk404/echoryn/internal/hivemind/service/plugin/builtin/memory/memory-core/embedding"
	entity2 "github.com/kiosk404/echoryn/internal/hivemind/service/plugin/builtin/memory/memory-core/entity"
	"github.com/kiosk404/echoryn/internal/hivemind/service/plugin/builtin/memory/memory-core/internal"
	hybrid2 "github.com/kiosk404/echoryn/internal/hivemind/service/plugin/builtin/memory/memory-core/internal/hybrid"
	"github.com/kiosk404/echoryn/internal/hivemind/service/plugin/builtin/memory/memory-core/internal/search"
	store2 "github.com/kiosk404/echoryn/internal/hivemind/service/plugin/builtin/memory/memory-core/store"
...
type Manager struct {
	cfg      *entity2.MemoryConfig
	provider embedding2.Provider
	db       *sql.DB
...
func Get(ctx context.Context, cfg *entity2.MemoryConfig) (*Manager, error) {
	key := cacheKey(cfg)
...
func newManager(ctx context.Context, cfg *entity2.MemoryConfig) (*Manager, error) {
	...
	providerResult, err := embedding2.NewProvider(cfg.Embedding)
	if err != nil {
	...
	var vecConfig *store2.VecSchemaConfig
	if cfg.Store.Vector.Enabled {
		vecConfig = &store2.VecSchemaConfig{
			Enabled:       true,
			Dimensions:    embeddingDimensions(cfg.Embedding.Model),
			ExtensionPath: cfg.Store.Vector.ExtensionPath,
		}
	}
	schemaResult, err := store2.EnsureSchema(db, ftsEnabled, vecConfig)
	if err != nil {
		db.Close()
	...
	prevProvider, _ := store2.GetMeta(db, store2.MetaKeyProvider)
	prevModel, _ := store2.GetMeta(db, store2.MetaKeyModel)
	providerKey := embedding2.ProviderKey(providerResult.Provider)
	...
	// Update meta.
	store2.SetMeta(db, store2.MetaKeyProvider, providerResult.Provider.ID())
	store2.SetMeta(db, store2.MetaKeyModel, providerResult.Provider.Model())
	_ = providerKey
...
func (m *Manager) Search(ctx context.Context, query string, opts ...SearchOption) ([]entity2.MemorySearchResult, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	...
	var vectorResults []hybrid2.VectorResult
	if len(queryVec) > 0 {
		if m.vecAvailable {
	...
	// Keyword search (FTS).
	var keywordResults []hybrid2.KeywordResult
	...
	var filtered []entity2.MemorySearchResult
...
func (m *Manager) Sync(ctx context.Context, opts SyncOpts) error {
	...
	fileCount, _ := store2.CountFiles(m.db)
	chunkCount, _ := store2.CountChunks(m.db)
...
func (m *Manager) runSync(ctx context.Context, opts SyncOpts) error {
	// Sync memory files.
	files, err := internal.ListMemoryFiles(m.cfg.WorkspaceDir, m.cfg.ExtraPaths)
	if err != nil {
	...
	for _, absPath := range files {
		entry, err := internal.BuildFileEntry(absPath, m.cfg.WorkspaceDir)
		if err != nil {
		...
		existingHash, found, _ := store2.GetFileRecord(m.db, entry.Path, entity2.MemorySourceMemory)
		if found && existingHash == entry.Hash && !opts.Force {
			continue
		}

		// Index this file.
		if err := m.indexFile(ctx, entry, entity2.MemorySourceMemory); err != nil {
			logger.Warn("[Memory] failed to index %s: %v", entry.Path, err)
		}
	}

	// Clean stale entries.
	stalePaths, _ := store2.GetStalePaths(m.db, entity2.MemorySourceMemory)
	for _, stalePath := range stalePaths {
		if _, ok := activePaths[stalePath]; !ok {
			store2.DeleteFileAndChunks(m.db, stalePath, entity2.MemorySourceMemory, m.provider.Model(), m.ftsAvailable)
		}
	}

	// Prune embedding cache.
	if m.cfg.Cache.Enabled && m.cfg.Cache.MaxEntries > 0 {
		store2.PruneEmbeddingCache(m.db, m.cfg.Cache.MaxEntries)
	}
...
func (m *Manager) indexFile(ctx context.Context, entry *entity2.MemoryFileEntry, source entity2.MemorySource) error {
	...
	// Chunk the content.
	chunks := internal.ChunkMarkdown(string(content), m.cfg.Chunking)
	if len(chunks) == 0 {
		return nil
	}
	...
	providerKey := embedding2.ProviderKey(m.provider)
	hashes := make([]string, len(chunks))
	for i, chunk := range chunks {
		hashes[i] = chunk.Hash
	}

	cachedEmbeddings, _ := store2.LoadEmbeddingCache(m.db, m.provider.ID(), m.provider.Model(), providerKey, hashes)
	...
	if m.vecAvailable {
		store2.DeleteVecChunksByPath(m.db, entry.Path, string(source))
	}
	store2.DeleteFileAndChunks(m.db, entry.Path, source, m.provider.Model(), m.ftsAvailable)
	...
	for i, chunk := range chunks {
		...
		if cached, ok := cachedEmbeddings[chunk.Hash]; ok {
			embeddingVec = internal.ParseEmbedding(cached)
			...
			if m.cfg.Cache.Enabled {
				...
				store2.UpsertEmbeddingCache(m.db, m.provider.ID(), m.provider.Model(), providerKey, chunk.Hash, string(embJSON), len(embeddingVec))
			}
		}

		embJSON, _ := json.Marshal(embeddingVec)
		if err := store2.InsertChunk(m.db, chunkID, entry.Path, source,
		...
		// Insert into FTS.
		if m.ftsAvailable {
			store2.InsertFTSChunk(m.db, texts[i], chunkID, entry.Path, source,
				m.provider.Model(), chunk.StartLine, chunk.EndLine)
		}

		// Insert into vec0 table.
		if m.vecAvailable && len(embeddingVec) > 0 {
			store2.InsertVecChunk(m.db, chunkID, embeddingVec)
		}
	}

	// Update file record.
	store2.UpsertFileRecord(m.db, entry, source)
...
func (m *Manager) DeleteMemory(relPath string) error {
	...
	// Clean up index.
	store2.DeleteFileAndChunks(m.db, relPath, entity2.MemorySourceMemory, m.provider.Model(), m.ftsAvailable)
...
func cacheKey(cfg *entity2.MemoryConfig) string {
...
func atomicClearIndex(db *sql.DB, ftsAvailable bool) error {
	stmts := []string{
		`DELETE FROM ` + store2.TableChunks,
		`DELETE FROM ` + store2.TableFiles,
		`DELETE FROM ` + store2.TableEmbeddingCache,
	}
	if ftsAvailable {
		stmts = append(stmts, `DELETE FROM `+store2.TableChunksFTS)
	}
	// Try to clear vec table (may not exist).
	stmts = append(stmts, `DELETE FROM `+store2.TableChunksVec)
