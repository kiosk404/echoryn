...
import (
	...
	"github.com/kiosk404/eidolon/pkg/utils/term"
...
func (a *App) runCommand(cmd *cobra.Command, args []string) error {
	...
	if !a.silence {
		logger.Info("%v Starting %s ...", progressMessage, a.name)
		if !a.noVersion {
			logger.Info("%v Version: `%s`", progressMessage, version.Get().ToJSON())
		}
		if !a.noConfig {
			logger.Info("%v Config file used: `%s`", progressMessage, viper.ConfigFileUsed())
		}
	}
	if a.options != nil {
...
func (a *App) applyOptionRules() error {
	...
	if printableOptions, ok := a.options.(PrintableOptions); ok && !a.silence {
		logger.Info("%v Config: `%s`", progressMessage, printableOptions.String())
	}
...
func printWorkingDir() {
	wd, _ := os.Getwd()
	logger.Info("%v WorkingDir: %s", progressMessage, wd)
}
