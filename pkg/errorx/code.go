...
var (
	unknownCoder defaultCoder = defaultCoder{1, http.StatusInternalServerError, "An internal server error occurred", "http://github.com/kiosk404/echoryn/pkg/errorx/README.md"}
)

// Coder defines an interface for an error code detail information.
type Coder interface {
	// HTTPStatus status that should be used for the associated error code.
	HTTPStatus() int

	// String (user) facing error text.
	String() string
...
func Register(coder Coder) {
	if coder.Code() == 0 {
		panic("code `0` is an unknownCode error code")
	}
...
func MustRegister(coder Coder) {
	if coder.Code() == 0 {
		panic("code '0' is reserved ErrUnknown error code")
	}
